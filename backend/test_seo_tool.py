...
async def run_tests():
    print(f"[{time.strftime('%H:%M:%S')}] 🔍 Testing SEO Scout...")
    ...
    """
    print(f"\n[{time.strftime('%H:%M:%S')}] 📋 Running Content Audit (Gemini)...")
    try:
        audit = await asyncio.wait_for(seo_scout.audit_content_for_aio(sample_content), timeout=30)
        print(json.dumps(audit, indent=2))
    except asyncio.TimeoutError:
        print("❌ Error: Content Audit timed out after 30 seconds.")
    except Exception as e:
        print(f"❌ Error: Content Audit failed: {e}")

    # 2. Test Keyword Scouting
    print(f"\n[{time.strftime('%H:%M:%S')}] 🌍 Scouting Keywords for 'Cappadocia' (Tavily + Gemini)...")
    try:
        ...
        print("❌ Error: Keyword Scouting timed out after 30 seconds.")
    except Exception as e:
        print(f"❌ Error: Keyword Scouting failed: {e}")

    print(f"\n[{time.strftime('%H:%M:%S')}] ✅ Tests Completed.")
    print(f"[{time.strftime('%H:%M:%S')}] 📝 Results summarized above.")
...
async def main():
    try:
        ...
        print("\n💥 CRITICAL ERROR: The entire test script timed out after 70 seconds.")
        sys.exit(1)
