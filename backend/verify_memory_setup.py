    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
             async with session.get(table_url, headers=headers) as r:
                if r.status == 200:
                    data = await r.json()
                    print(f"✅ Table accessible. Found {len(data)} entries.")
                    for item in data:
                        print(f"   - [{item.get('created_at', 'No Date')}] {item.get('problem_title', 'No Title')}")
                else:
                    text = await r.text()
                    print(f"❌ Table check failed: {r.status} - {text}")
        except Exception as e:
            print(f"❌ Table check exception: {e}")
            ...
            async with session.get(f"{SUPABASE_URL}/rest/v1/", headers=headers) as r:
                if r.status == 200:
                    ...
                    if '/rpc/match_developer_knowledge' in paths:
                         print("✅ RPC 'match_developer_knowledge' is exposed.")
                    else:
                         print("⚠️ RPC 'match_developer_knowledge' not found in OpenAPI spec.")
                         ...
                         async with session.post(rpc_url, headers=headers, json={}) as rpc_r:
                             if rpc_r.status != 404: # 404 means not found, anything else (like 400 bad request) means it exists
                                 print("✅ RPC 'match_developer_knowledge' exists (responded to call).")
                             else:
                                 print("❌ RPC 'match_developer_knowledge' appears missing (404).")
                else:
                    print(f"⚠️ Could not fetch OpenAPI spec: {r.status}")
        except Exception as e:
             print(f"❌ RPC check exception: {e}")
