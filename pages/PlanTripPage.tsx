...
import { Sparkles, MapPin, Calendar, Compass, Send, LoaderCircle, ExternalLink, Info } from 'lucide-react';

const PlanTripPage = () => {
  const [destination, setDestination] = useState('');
  const [duration, setDuration] = useState('3');
  const [itinerary, setItinerary] = useState<{ text: string; sources: any[] } | null>(null);
        ...
        <section className="bg-primary text-white py-16 px-4">
          <div className="container mx-auto max-w-4xl text-center">
            <div className="inline-flex items-center gap-2 bg-white/10 px-4 py-1.5 rounded-full border border-white/20 mb-6 backdrop-blur-sm">
              ...
              <span className="text-sm font-semibold uppercase tracking-wider">AI Powered Planning</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold font-serif mb-6 leading-tight">
              Where will your next <span className="text-blue-300 italic">adventure</span> take you?
            </h1>
            <p className="text-lg text-blue-100 mb-10 max-w-2xl mx-auto">
              Our AI travel consultant uses real-time Google data to build the perfect day-by-day itinerary just for you.
            </p>

            <form onSubmit={handlePlanTrip} className="bg-white p-2 rounded-2xl shadow-2xl flex flex-col md:flex-row gap-2 max-w-3xl mx-auto">
              ...
              <button 
                type="submit" 
                disabled={isLoading}
                className="bg-accent hover:bg-orange-600 text-white px-8 py-3 rounded-xl font-bold transition flex items-center justify-center gap-2 shrink-0 disabled:bg-gray-400"
              >
        ...
        <section className="container mx-auto max-w-6xl px-4 -mt-10">
          {!itinerary && !isLoading && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-80">
                {[
                  { icon: Compass, title: 'Local Insights', desc: 'Hidden gems suggested by real-time web searches.' },
                  { icon: MapPin, title: 'Map Grounding', desc: 'Direct links to verified Google Maps locations.' },
                  { icon: Sparkles, title: 'Personalized', desc: 'Tailored day-by-day flows for your time frame.' }
                ].map((item, i) => (
          ...
          {isLoading && (
            <div className="bg-white rounded-3xl shadow-xl p-12 text-center border border-blue-50">
              ...
              <h2 className="text-2xl font-bold text-neutral mb-2">Building your dream trip...</h2>
              <p className="text-gray-500 max-w-md mx-auto">
                Gemini is currently cross-referencing maps, reviews, and latest travel guides to find the best spots in {destination}.
              </p>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                  <div className="bg-primary p-6 text-white flex items-center justify-between">
                    <div>
                      <h2 className="text-2xl font-bold font-serif">Your {duration}-Day Trip to {destination}</h2>
                      <p className="text-blue-100 text-sm mt-1">Generated by Gemini Intelligence</p>
                    </div>
                  ...
                  <div className="space-y-4">
                    {itinerary.sources.filter(s => s.maps).length > 0 ? (
                      itinerary.sources.map((source, i) => (
                        source.maps && (
                          <div key={i} className="group p-4 bg-gray-50 rounded-2xl hover:bg-blue-50 transition-colors border border-transparent hover:border-blue-100">
                            <h4 className="font-bold text-neutral text-sm group-hover:text-primary transition-colors">{source.maps.title}</h4>
                            <a 
                      ...
                      <div className="p-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p className="text-xs text-gray-400">Search grounded locations will appear here.</p>
                      </div>
                    )}
                  </div>

                  <div className="mt-8 pt-6 border-t border-gray-100">
                     <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mb-4">Travel Tip</p>
                     <div className="bg-blue-50 p-4 rounded-xl text-xs text-blue-800 leading-normal">
